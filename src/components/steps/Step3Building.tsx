...
import { Home, Building, Building2, HelpCircle, ArrowLeft } from 'lucide-react';

interface Step3Props {
  data?: string;
  onNext: (building: string) => void;
  onBack: () => void;
}

export default function Step3Building({ data, onNext, onBack }: Step3Props) {
  const options = [
    { value: 'Einfamilienhaus', label: 'Einfamilienhaus', icon: Home, desc: 'Freistehendes Haus' },
    { value: 'Mehrfamilienhaus', label: 'Mehrfamilienhaus', icon: Building, desc: 'Wohnblock / mehrere Parteien' },
    { value: 'Reihenhaus', label: 'Reihenhaus / DHH', icon: Building2, desc: 'Reiheneck- oder Doppelhaushälfte' },
    { value: 'Anderes', label: 'Anderes Gebäude', icon: HelpCircle, desc: 'Gewerbe, Denkmal oder Sonstiges' },
  ];
    ...
    <div className="w-full">
      <div className="text-center mb-8">
        ...
        <p className="text-sm text-text-muted mt-2">
          Der Gebäudetyp hilft bei der Abschätzung des Wärmebedarfs und der passenden Wärmepumpenleistung.
        </p>
      </div>
